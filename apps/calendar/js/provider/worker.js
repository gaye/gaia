...
exports.ensureRecurrencesExpanded = function(options) {
  ...
  return icalComponentStore.findRecurrencesBefore(maxDate).then(components => {
...
exports.createEvent = function(options) {
  ...
  .then((remote) => {
    ...
    return new Promise((resolve, reject) => {
      var formatted = formatEvent(event, remote, recurring);
      debug('Will issue event mutations:', formatted);
      var create = mutations.create(formatted);
      create.commit((err) => {
        if (err) {
          return reject(err);
        }
...
function createServiceError(err, details) {
  ...
  switch (err.name) {
    case 'caldav-authentication':
      error = new CalendarError.Authentication(details);
      break;
    case 'caldav-invalid-entrypoint':
      error = new CalendarError.InvalidServer(details);
      break;
    case 'caldav-server-failure':
      error = new CalendarError.ServerFailure(details);
      break;
    default:
      error = new CalendarError(err.name, details);
      break;
  }

  // TODO(gareth): This marking is a weird side-effect... actually this whole
  //     thing is weird?
  if (error instanceof CalendarError.Authentication ||
